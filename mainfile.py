...
wn.setup(2000,700)                # setup the dimensions of the working window
wn.addshape('img.gif')
...
def search(x,y):
    ...
    while len(frontier) > 0:          # exit while loop when frontier queue equals zero
        ...
        if(x - 24, y) in path and (x - 24, y) not in visited:  # check the cell on the left
            ...
            blue.stamp()
            if(x-24 == end_x and y == end_y): break 
        ...
        if (x, y - 24) in path and (x, y - 24) not in visited:  # check the cell down
            ...
            blue.goto(cell)
            blue.stamp()
            if(x == end_x and y-24 == end_y): break
        ...
        if(x, y + 24) in path and (x, y + 24) not in visited:  # check the cell up
            ...
            frontier.append(cell)
            visited.add((x, y + 24))
        green.goto(x,y) # we have explored all the neighbours of x,y and marked as explored
        green.stamp()
